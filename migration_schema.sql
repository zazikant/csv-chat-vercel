-- =====================================================================
--  Email Campaign Tracker - Schema v2
--  Target: gdmztlzpsobyjousvvyp.supabase.co  (NEW project)
--  Run in: Supabase Dashboard -> SQL Editor -> New query
-- =====================================================================
--
--  DESIGN v2 (simplified, no auto-mailer-sending):
--
...
--    - opens       (int, manually entered by user)
--    - clicks      (int, manually entered by user)
--    - unsubscribed(bool, manually toggled)
...
--    - unsubscribed      (auto: count of contacts with this mailer_id AND unsubscribed=true)
--    - open_rate         (GENERATED: unique_opens / total_sent * 100)
--    - click_rate        (GENERATED: unique_clicks / total_sent * 100)
--    - unsubscribe_rate  (GENERATED: unsubscribed / total_sent * 100)
--
--  Auto-rollup trigger: AFTER INSERT/UPDATE/DELETE on contacts,
--  recompute the affected mailer's counters from scratch.
--
--  email_journey table is REMOVED — the app does not send auto mailers,
--  so per-contact manual counters on `contacts` are sufficient.
--
...
drop function if exists public.recompute_rollups();
...
-- ---------------------------------------------------------------------
-- 2. mailers  (campaign library - created first because contacts references it)
-- ---------------------------------------------------------------------
...
comment on column public.mailers.unsubscribed_count is 'Auto: count of contacts with this mailer_id AND unsubscribed = true.';
...
comment on table  public.contacts is 'Master contact list. One row per person. Each contact is assigned to at most one mailer at a time; opens/clicks/unsubscribed are manually maintained per (contact, mailer).';
...
comment on column public.contacts.unsubscribed is 'True if this contact unsubscribed via this mailer (manually toggled).';
...
--    affected mailers' counters from scratch. (If mailer_id changes,
--    both the old and new mailers are recomputed.)
-- ---------------------------------------------------------------------
...
begin
    ...
    set
        ...
        unsubscribed_count = (select count(*)           from public.contacts c where c.mailer_id = m.mailer_id and c.unsubscribed = true),
        updated_at         = now()
    where m.mailer_id = p_mailer_id;
end;
$$;

-- Per-row trigger: handles INSERT/UPDATE/DELETE on contacts
...
begin
    ...
    -- Also touch last_activity_date and engagement_score on the new row
    ...
    -- Recompute the affected mailer(s)
...
-- ---------------------------------------------------------------------
-- 5. updated_at auto-touch trigger (keep updated_at fresh on direct updates)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 7. Optional: run_select_query RPC for the chat panel
