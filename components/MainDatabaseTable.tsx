...
export default function MainDatabaseTable({
  ...
  function downloadCSV() {
    ...
    const rowsData = filteredRows.map((row) => [...exportCols.map((c) => {
      const val = row[c.key];
    ...
    const a = document.createElement("a"); a.href = url; a.download = `main_database_${new Date().toISOString().slice(0, 10)}.csv`; a.click(); URL.revokeObjectURL(url);
  }
    ...
    <div className="flex flex-col h-full">
      ...
      {showFilters && (
        <div className="px-2 sm:px-4 py-3 border-b border-gray-200 bg-gray-50/70 flex flex-wrap items-end gap-3">
          ...
          <FilterSelect label="Sector" value={filters.sector} onChange={(v) => { setFilters((f) => ({ ...f, sector: v })); onPageChange(1); }} options={[{ value: "", label: "Any sector" }, ...sectorOptions.map((s) => ({ value: s, label: s }))]} />
          <FilterSelect label="Source" value={filters.source} onChange={(v) => { setFilters((f) => ({ ...f, source: v })); onPageChange(1); }} options={[{ value: "", label: "Any source" }, ...sourceOptions.map((s) => ({ value: s, label: s }))]} />
          <FilterSelect label="Tag" value={filters.tag} onChange={(v) => { setFilters((f) => ({ ...f, tag: v })); onPageChange(1); }} options={[{ value: "", label: "Any tag" }, ...tagOptions.map((t) => ({ value: t, label: t }))]} />
