...
interface Filters {
  mailer_id: string;          // "" = any, "(none)" = NULL
  optin_status: string;       // "" = any
  engagement_score: string;   // "" = any
  city: string;
  sector: string;
  tag: string;                // free text, matches any tag in tags[]
}

const EMPTY_FILTERS: Filters = {
  mailer_id: "", optin_status: "", engagement_score: "", city: "", sector: "", tag: "",
};
...
export default function ContactsTable({
  ...
  const sectorOptions = Array.from(new Set(rows.map((r) => r.sector).filter((x): x is string => !!x))).sort();
  ...
  const filteredRows = rows.filter((row) => {
    ...
    if (searchTerm.trim()) {
      const matchesSearch = SEARCHABLE_COLUMNS.some((col) => {
        const val = row[col];
    ...
    } else if (filters.mailer_id && row.mailer_id !== filters.mailer_id) {
      return false;
    }
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-white">
        ...
        <div className="flex items-center gap-2">
          ...
          {(searchTerm || isFiltered || activeFilterCount > 0) && filteredRows.length > 0 && (
          ...
          <button onClick={onBulkDelete} className="px-3 py-1.5 text-xs border border-red-200 hover:bg-red-50 text-red-600 rounded-lg transition-colors flex items-center gap-1.5" title="Bulk delete contacts via CSV or pasted email list">
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
            </svg>
            Delete CSV
      ...
      {showFilters && (
        <div className="px-4 py-3 border-b border-gray-200 bg-gray-50/70 flex flex-wrap items-center gap-2">
          <FilterSelect
            label="Mailer"
            value={filters.mailer_id}
            onChange={(v) => { setFilters((f) => ({ ...f, mailer_id: v })); onPageChange(1); }}
            options={[
              { value: "", label: "Any mailer" },
              { value: "(none)", label: "(unassigned)" },
              ...mailerOptions.map((m) => ({ value: m, label: m })),
            ]}
          />
          <FilterSelect
            label="Opt-in"
            value={filters.optin_status}
            onChange={(v) => { setFilters((f) => ({ ...f, optin_status: v })); onPageChange(1); }}
            options={[
              { value: "", label: "Any status" },
              { value: "Subscribed", label: "Subscribed" },
              { value: "Hard Bounced", label: "Hard Bounced" },
              { value: "Unsubscribed", label: "Unsubscribed" },
            ]}
          />
          <FilterSelect
            label="Engagement"
            value={filters.engagement_score}
            onChange={(v) => { setFilters((f) => ({ ...f, engagement_score: v })); onPageChange(1); }}
            options={[
              { value: "", label: "Any engagement" },
              { value: "HOT", label: "HOT (clicked)" },
              { value: "WARM", label: "WARM (opened)" },
              { value: "COLD", label: "COLD (no activity)" },
            ]}
          />
          <FilterSelect
            label="City"
            value={filters.city}
            onChange={(v) => { setFilters((f) => ({ ...f, city: v })); onPageChange(1); }}
            options={[
          ...
          <FilterSelect
            ...
            onChange={(v) => { setFilters((f) => ({ ...f, sector: v })); onPageChange(1); }}
            options={[
          ...
          <FilterSelect
            label="Tag"
            value={filters.tag}
            onChange={(v) => { setFilters((f) => ({ ...f, tag: v })); onPageChange(1); }}
            options={[
              { value: "", label: "Any tag" },
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <select
        ...
        className="px-2 py-1.5 text-xs text-gray-900 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-100 focus:border-blue-300 bg-white min-w-[120px]"
      >
        {options.map((o) => (
